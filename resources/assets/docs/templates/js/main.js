$(function(){
	...
	$('.api-explorer-form').on('submit', function(e){
		...
		var uriAction = uriArray[uriArray.length - 1];
		var cookieToken = Cookies.get('test_api_token') ? Cookies.get('test_api_token') : '';
		...
		var dataArray =	self.serialize().split('&');
		...
		var data = dataArray.filter(function(item) {
	 	...
	 	$.ajax({
			...
			{
				request.setRequestHeader('Authorization', 'Bearer ' + cookieToken)
			},
			success: function(data, status, request)
	        {
				...
				if (uriAction === 'login') {
					Cookies.set('test_api_token', JSON.parse(request.responseText).token);
				}

				if (uriAction === 'logout') {
					Cookies.remove('test_api_token');
				}
		    },
